...
"""Prepare and sample from a W-state using the QDMI-on-IQM stack."""
...
if __name__ == "__main__":
    ...
    log.info(
        "Starting W-state example (backend=%s, qubits=%d, shots=%d)",
        args.backend,
        args.num_qubits,
        args.shots,
    )
    ...
    if backend.num_qubits < args.num_qubits:
        sys.exit(
            f"Selected backend exposes {backend.num_qubits} qubits, but the W-state example needs {args.num_qubits}."
        )

    log.info("Building W-state benchmark circuit (n=%d)...", args.num_qubits)
    ...
    expected_counts = {
        f"{1 << (args.num_qubits - index - 1):0{args.num_qubits}b}": args.shots / args.num_qubits
        for index in range(args.num_qubits)
    }
