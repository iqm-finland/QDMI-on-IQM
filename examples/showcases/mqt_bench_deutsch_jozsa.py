...
"""Run a balanced Deutsch-Jozsa showcase through the IQM QDMI Qiskit backend."""
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the Deutsch-Jozsa showcase.
...
def main() -> None:
    """Execute the balanced Deutsch-Jozsa showcase.

    Raises:
        SystemExit: If backend setup fails or the showcase result does not satisfy the validation threshold.
    """
    args = parse_args()
    ...
    if backend.num_qubits < args.num_qubits:
        msg = (
            ...
            f"showcase needs {args.num_qubits}."
        )
        raise SystemExit(msg)
