...
"""Run the Quantum Phase Estimation algorithm using the QDMI-on-IQM stack."""
...
if __name__ == "__main__":
    ...
    log.info(
        "Starting Quantum Phase Estimation example (backend=%s, qubits=%d, shots=%d)",
        args.backend,
        args.num_qubits,
        args.shots,
    )
    ...
    if backend.num_qubits < args.num_qubits:
        sys.exit(f"Selected backend exposes {backend.num_qubits} qubits, but the QPE example needs {args.num_qubits}.")

    log.info("Building and mapping Quantum Phase Estimation benchmark circuit (n=%d)...", args.num_qubits)
